...
def load_spline_params():
  with open('robustdepthflow/misc/partition_spline.npz', "rb") as spline_file:
...
def general_adaptive_loss(x, shape, bowl=1.):
  ...
  x = (torch.abs(shape - 2)/shape) * (torch.pow((torch.square(x/bowl)/torch.abs(shape - 2) + 1), shape/2) - 1) + log_safe(bowl) + log_safe(partition)
