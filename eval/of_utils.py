...
def compute_of_metrics(pred_ofs, gt_ofs):
    ...
    epe = torch.norm(pred_ofs_resized - gt_ofs, p=2, dim=1)
    gt_ofs_norm = torch.norm(gt_ofs, p=2, dim=1)
...
def compute_metrics_cpu(pred_ofs, gt_ofs):
    ...
    for i in range(len(pred_ofs)):
        ...
        error_rates.append(np.logical_and(
            epe > 3.0, epe > 0.05 * gt_ofs_norm).astype(np.float32))
