...
def parse_range_image_and_camera_projection(frame):
    ...
    for laser in frame.lasers:
        if len(
                laser.ri_return1.range_image_compressed) > 0:  # pylint: disable=g-explicit-length-test
        ...
        if len(
                laser.ri_return2.range_image_compressed) > 0:  # pylint: disable=g-explicit-length-test
            range_image_str_tensor = tf.io.decode_compressed(
...
def convert_frame_to_dict(frame):
    ...
    # Save the intrinsics, 4x4 extrinsic matrix, width, and height of each
    # camera.
