...
def encode_lidar_features(lidar_point_feature):
    ...
    return tf.stack([encoded_r, encoded_intensity,
                    encoded_elongation], axis=-1)
...
def decode_lidar_features(lidar_point_feature):
    ...
    decoded_elongation = _decode_elongation(
        tf.cast(elongation, dtype=tf.uint8))
...
def compute_range_image_polar(range_image,
    ...
    with tf.compat.v1.name_scope(scope, 'ComputeRangeImagePolar',
        ...
        with tf.compat.v1.name_scope('Azimuth'):
            # [B].
            az_correction = tf.atan2(
                extrinsic[..., 1, 0], extrinsic[..., 0, 0])
            # [W].
            ratios = (tf.cast(tf.range(width, 0, -1), dtype=dtype) - .5) / tf.cast(
                width, dtype=dtype)
            # [B, W].
            azimuth = (ratios * 2. - 1.) * np.pi - \
                tf.expand_dims(az_correction, -1)

        # [B, H, W]
        azimuth_tile = tf.tile(azimuth[:, tf.newaxis, :], [1, height, 1])
        # [B, H, W]
        inclination_tile = tf.tile(
            inclination[:, :, tf.newaxis], [1, 1, width])
...
def compute_range_image_cartesian(range_image_polar,
    ...
    with tf.compat.v1.name_scope(
        ...
        translation = tf.expand_dims(
            tf.expand_dims(extrinsic[..., 0:3, 3], 1), 1)
...
def build_camera_depth_image(range_image_cartesian,
    ...
    with tf.compat.v1.name_scope(
        ...
        def fn(args):
            ...
            return scatter_nd_with_pool(
                index, value, camera_image_size, pool_method)
...
def build_range_image_from_point_cloud(points_vehicle_frame,
    ...
    with tf.compat.v1.name_scope(
        ...
        # [B, N], within [-2pi, 2pi]
        point_azimuth = tf.atan2(
            points[..., 1], points[..., 0]) + az_correction
        ...
        with tf.control_dependencies([
            ...
            # [B, N, 2]
            ri_indices = tf.stack(
                [point_ri_row_indices, point_ri_col_indices], -1)
            # [B, N]
            ri_ranges = tf.cast(
